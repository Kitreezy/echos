...
//  echosTests
//
//  Один и тот же набор сценариев против разных реализаций транспорта.
//
//  Это и есть сравнение «нативно против Starscream»: не пересказ README,
//  а вопрос «что упало». Базовый класс гоняет `URLSessionRawSocket`,
//  наследники подменяют фабрику — код тестов при этом не меняется ни на строку.
//
...
class WebSocketContractTests: XCTestCase {
    ...
    /// Имя конфигурации — попадает в сообщения об ошибках, иначе непонятно,
    /// какая из трёх реализаций упала.
...
// MARK: - Starscream, движок по умолчанию

/// `useCustomEngine: true` — собственный фрейминг Starscream поверх
/// Network.framework. Именно это получает тот, кто просто подключил библиотеку.
@MainActor
final class StarscreamWSEngineContractTests: WebSocketContractTests {

    override func makeRawSocket(url: URL) -> any RawWebSocket {
        StarscreamRawSocket(url: url, engine: .starscream)
    }

    override var configurationName: String {
        "Starscream/WSEngine"
    }
}

// MARK: - Starscream поверх URLSessionWebSocketTask

/// `useCustomEngine: false` — Starscream оборачивает тот же самый
/// `URLSessionWebSocketTask`, на котором написан `URLSessionRawSocket`.
/// Сравнение с базовым классом показывает цену именно обёртки.
@MainActor
final class StarscreamNativeEngineContractTests: WebSocketContractTests {

    override func makeRawSocket(url: URL) -> any RawWebSocket {
        StarscreamRawSocket(url: url, engine: .native)
    }
