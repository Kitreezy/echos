...
final class RelayServer {
    ...
    /// Сколько клиентов уже представились. Отличается от числа соединений:
    /// пока клиент не прошёл рукопожатие, отпечатка у него нет, и адресный
    /// конверт до него не доходит — релей такого не обслуживает.
    var introducedClientCount: Int {
        clients.values.filter { $0.fingerprint != nil }.count
    }
