...
Чек-лист — [docs/review-checklist.md](docs/review-checklist.md), восемь
пунктов про то, что в echos действительно ломается и что тесты не ловят:
изоляция, что переживает переподключение, что видит релей, ключи,
поведение без теста, комментарий против кода, совместимость по проводу,
решение без ADR. У каждого — почему он тут и по какому признаку его узнать.
Стиля и именования в нём нет намеренно: это тема линтера, а не ревью.

Применяется в двух местах одним и тем же текстом. Локально —
`scripts/review.sh`: diff к `origin/develop`, чек-лист, `claude -p`, ответ
по пунктам с файлом и строкой; замечание без строки не считается. С
диапазоном `A..B` смотрит старый PR — и делает это в временной копии на
верхнем коммите диапазона, потому что модель заглядывает в код через
`Read`, и код обязан быть той версии, чей diff она читает. Первая
калибровка это и показала: ревью #27 сообщило «в `docs/adr` только 001»,
прочитав старую ветку. В CI — job `review` читает тот же файл.

Калибровалось на трёх влитых PR. Что нашлось и подтвердилось: в #27
атрибуты реакций добавлены в единственную версию модели Core Data — у
установленной сборки после обновления `loadPersistentStores` уйдёт в
`fatalError`; снятие реакции приезжает старой сборке пустым сообщением;
проверка отправителя реакции пропускает сообщения без адреса. В #12 записи
в `ciphers`, `previousCiphers` и `cipherInputs` никогда не удаляются —
ушедший собеседник живёт в них до конца транспорта, `noCipher` для него
больше не бросается, а `ciphersAreEmpty`, на котором стоят ожидания в
тестах, после первого собеседника не бывает `true`; сессионный ключ релея
не забывается при разрыве вопреки ADR 004. В #11 — гонка в `makePair()`,
та самая, которую потом убрал `check.sh`, и тест, который проходит, ничего
не проверяя. Всё это ушло в задачи.

Что оказалось шумом и поправлено в чек-листе. Пункт про релей ругался на
`typing`, `stroke` и `wallState` в каждом PR, хотя ADR 004 сознательно
оставил их открытыми, — теперь пункт это знает. Пункт про совместимость
считал обязательные поля в `HelloPayload` замечанием, хотя порядок выкатки
«сначала релей» записан, — теперь допускает. Туда же добавлен признак про
версию модели Core Data: он всплыл в калибровке, а в чек-листе его не было.
Остальные замечания — расхождения комментариев с кодом, недостижимая ветка,
`TODO`, оставшийся в коммите, который его закрыл, — мелкие, но не ложные.
